...
def calculate_hero_stats(m_hero_df):
    """From hero df, creates total_matches, h_pickrate, h_winrate"""
    def get_total_matches(df):
        """calculates total matches for each hero"""
...
def split_players_from_matches(df)-> pd.DataFrame:
    """For a match df, extracts players into new dataframe.
        
        returns the match_df and players_df
        players df contains the match_id they were extracted from"""
...
def match_data_normalize(json)->pd.DataFrame:
    """Normalized json and return as DataFrame"""
    df = pd.json_normalize(json, record_path="players", meta=["match_id", "winning_team"])
    return df

def match_data_outcome_add(df)-> pd.DataFrame:
    """compares 'won' to 'team' to calculate bool 'winning_team'"""
